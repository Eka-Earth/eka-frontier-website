      ...
      {#each engagementModels as model, index}
        ...
        >
          <div class="relative p-10">
            ...
            <h3 class="text-2xl font-bold text-white mb-3 group-hover:text-eka-sage transition-all duration-300">
              {model.title}
            </h3>
          ...
          <!-- Bottom accent line -->
          <div class="absolute bottom-0 left-0 right-0 h-0.5 bg-eka-primary transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left"></div>
        </div>
      {/each}
    </div>
