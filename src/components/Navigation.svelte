...
>
  <div class="max-w-7xl mx-auto px-6 lg:px-12">
    <div class="flex justify-between items-center py-3">
      ...
      <button class="relative z-[60] p-2" on:click={toggleMenu} aria-label="Toggle menu">
        <div class="w-8 h-6 relative flex flex-col justify-center">
          <span
            class="absolute w-full h-0.5 {scrolled || isOpen
              ? 'bg-eka-primary-dark'
              : 'bg-white'} transition-all duration-300 {isOpen ? 'rotate-45 top-[11px]' : 'top-1'}"
          ></span>
          <span
            class="absolute w-full h-0.5 {scrolled || isOpen
              ? 'bg-eka-primary-dark'
              : 'bg-white'} transition-all duration-300 {isOpen ? 'opacity-0' : 'top-[11px]'}"
          ></span>
          <span
            class="absolute w-full h-0.5 {scrolled || isOpen
              ? 'bg-eka-primary-dark'
              : 'bg-white'} transition-all duration-300 {isOpen
              ? '-rotate-45 top-[11px]'
              : 'top-5'}"
          ></span>
        </div>
    ...
    <div class="fixed inset-0 bg-white z-[55] animate-fade-in">
      <div class="flex flex-col justify-center items-start h-full max-w-7xl mx-auto px-12 lg:px-24">
        <div class="space-y-8">
          ...
          <div
            class="pt-8 mt-8 border-t border-gray-200 animate-slide-up"
